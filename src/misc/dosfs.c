...
u32 DFS_GetPtnStart(u8 unit, u8 *scratchsector, u8 pnum, u8 *pactive, u8 *pptype, u32 *psize)
{
	...
	result = (u32) mbr->ptable[pnum].start_0 |
		   (((u32) mbr->ptable[pnum].start_1) << 8) |
		   (((u32) mbr->ptable[pnum].start_2) << 16) |
		   (((u32) mbr->ptable[pnum].start_3) << 24);

	if (pactive) *pactive = mbr->ptable[pnum].active;

	if (pptype) *pptype = mbr->ptable[pnum].type;
...
//	volinfo->oemid[8] = 0;
	...
	volinfo->reservedsecs = (u16) lbr->bpb.reserved_l |
		  				  (((u16) lbr->bpb.reserved_h) << 8);

	volinfo->numsecs =  (u16) lbr->bpb.sectors_s_l |
		 			  (((u16) lbr->bpb.sectors_s_h) << 8);

	if (!volinfo->numsecs)
	{
		volinfo->numsecs = (u32) lbr->bpb.sectors_l_0 |
						 (((u32) lbr->bpb.sectors_l_1) << 8) |
						 (((u32) lbr->bpb.sectors_l_2) << 16) |
						 (((u32) lbr->bpb.sectors_l_3) << 24);
	}
	...
	volinfo->secperfat =  (u16) lbr->bpb.secperfat_l |
		  				(((u16) lbr->bpb.secperfat_h) << 8);

	if (!volinfo->secperfat) 
	{
		volinfo->secperfat = (u32) lbr->ebpb.ebpb32.fatsize_0 |
						   (((u32) lbr->ebpb.ebpb32.fatsize_1) << 8) |
						   (((u32) lbr->ebpb.ebpb32.fatsize_2) << 16) |
						   (((u32) lbr->ebpb.ebpb32.fatsize_3) << 24);
...
//		volinfo->system[8] = 0; 
	...
	volinfo->rootentries =  (u16) lbr->bpb.rootentries_l |
		  				  (((u16) lbr->bpb.rootentries_h) << 8);

	// after extracting raw info we perform some useful precalculations
	volinfo->fat1 = startsector + volinfo->reservedsecs;

	//kprintf("DFS_GetVolInfo: fat1: %lu - startsector: %lu - reservedsecs: %u", volinfo->fat1, startsector, volinfo->reservedsecs);
	...
	if (volinfo->numclusters < 4085) volinfo->filesystem = FAT12;
	else if (volinfo->numclusters < 65525) volinfo->filesystem = FAT16;
	else volinfo->filesystem = FAT32;
...
*/
u32 DFS_GetFAT(PVOLINFO volinfo, u8 *scratch, u32 *scratchcache, u32 cluster)
{
	u32 offset, sector, result;
	...
	else return 0x0ffffff7;	// FAT32 bad cluster	
	...
	if (volinfo->filesystem == FAT12) 
	{
		...
		if (offset == SECTOR_SIZE - 1) 
		{
			...
			*scratchcache = sector;
			...
			result |= ((u32) scratch[0]) << 8;
		}
		else 
		{
			result = (u32) scratch[offset] |
			  ((u32) scratch[offset+1]) << 8;
		}
		if (cluster & 1)
			result = result >> 4;
		else
	...
	else if (volinfo->filesystem == FAT32) 
	{
		result = ((u32) scratch[offset] |
		  		 ((u32) scratch[offset+1]) << 8 |
		  		 ((u32) scratch[offset+2]) << 16 |
		  		 ((u32) scratch[offset+3]) << 24) & 0x0fffffff;
	}
	else
...
*/
u32 DFS_SetFAT(PVOLINFO volinfo, u8 *scratch, u32 *scratchcache, u32 cluster, u32 new_contents)
{
	u32 offset, sector, result;
	...
	else return DFS_ERRMISC;	

	// at this point, offset is the BYTE offset of the desired sector from the start
	// of the FAT. Calculate the physical sector containing this FAT entry.
	sector = ldiv(offset, SECTOR_SIZE).quot + volinfo->fat1;

	//kprintf("DFS_SetFAT - scratchcache: %lu - offset: %lu - sze: %u - sector: %lu - q: %ld - fat1: %lu", *scratchcache, offset, SECTOR_SIZE, sector, ldiv(offset, SECTOR_SIZE).quot, volinfo->fat1);
	...
	//kprintf("DFS_SetFAT - scratchcache: %lu", *scratchcache);
	...
	if (volinfo->filesystem == FAT12) 
	{
		...
		if (cluster & 1) new_contents = new_contents << 4;
		...
		if (offset == SECTOR_SIZE - 1) 
		{
			...
			if (DFS_OK == result) result = DFS_WriteSector(volinfo->unit, scratch, (*scratchcache)+volinfo->secperfat, 1);
		...
		else 
		{
			...
			//kprintf("DFS_SetFAT - scratch0: %u - scratch1: %u - offset: $%lX", scratch[offset], scratch[offset+1], offset);
...
*/
u8 *DFS_CanonicalToDir(u8 *dest, u8 *src)
{
	u8 *destptr = dest;
...
*/
u32 DFS_GetFreeFAT(PVOLINFO volinfo, u8 *scratch)
{
	u32 i, result = 0xffffffff, scratchcache = 0;
...
u32 DFS_OpenDir(PVOLINFO volinfo, u8 *dirname, PDIRINFO dirinfo)
{
	...
	else 
	{
		...
		if (volinfo->filesystem == FAT32) 
		{
			...
			// read first sector of directory
			if (DFS_ReadSector(volinfo->unit, dirinfo->scratch, volinfo->dataarea + ((volinfo->rootdir - 2) * volinfo->secperclus), 1)) return DFS_ERRMISC;
		}
		else 
		{
			dirinfo->currentcluster = 0;
			...
			// read first sector of directory
			if (DFS_ReadSector(volinfo->unit, dirinfo->scratch, volinfo->rootdir, 1)) return DFS_ERRMISC;
		}

		// skip leading path separators
		while (*ptr == DIR_SEPARATOR && *ptr) ptr++;
		...
		while (*ptr) 
		{
			...
			if (!memcmp(de.name, tmpfn, 11) && ((de.attr & ATTR_DIRECTORY) == ATTR_DIRECTORY)) 
			{
				if (volinfo->filesystem == FAT32) 
				{
					dirinfo->currentcluster = (u32) de.startclus_l_l |
										     ((u32) de.startclus_l_h) << 8 |
											 ((u32) de.startclus_h_l) << 16 |
											 ((u32) de.startclus_h_h) << 24;
				}
				else 
				{
					dirinfo->currentcluster = (u32) de.startclus_l_l |
				...
				if (DFS_ReadSector(volinfo->unit, dirinfo->scratch, volinfo->dataarea + ((dirinfo->currentcluster - 2) * volinfo->secperclus), 1)) return DFS_ERRMISC;
			}
			else if (!memcmp(de.name, tmpfn, 11) && !(de.attr & ATTR_DIRECTORY)) return DFS_NOTFOUND;

			// seek to next item in list
			while (*ptr != DIR_SEPARATOR && *ptr) ptr++;

			if (*ptr == DIR_SEPARATOR) ptr++;
		}
...
{
	u32 tempint;	// required by DFS_GetFAT
	...
	if (dirinfo->currententry >= SECTOR_SIZE / sizeof(DIRENT)) 
	{
		...
		if (dirinfo->currentcluster == 0) 
		{
			...
			if (dirinfo->currentsector * (SECTOR_SIZE / sizeof(DIRENT)) >= volinfo->rootentries) return DFS_EOF;

			// Otherwise try to read the next sector
			if (DFS_ReadSector(volinfo->unit, dirinfo->scratch, volinfo->rootdir + dirinfo->currentsector, 1)) return DFS_ERRMISC;
		}
		...
		else 
		{
			if (dirinfo->currentsector >= volinfo->secperclus) 
			{
				    ...
				    (dirinfo->currentcluster >= 0x0ffffff7 &&  volinfo->filesystem == FAT32)) 
				{
				  	...
				  	// to see.
				  	if (!(dirinfo->flags & DFS_DI_BLANKENT)) return DFS_EOF;
					
					// On the other hand, if this is a "find free entry" search,
					// we need to tell the caller to allocate a new cluster
					else return DFS_ALLOCNEW;
				}

				dirinfo->currentcluster = DFS_GetFAT(volinfo, dirinfo->scratch, &tempint, dirinfo->currentcluster);
			}

			if (DFS_ReadSector(volinfo->unit, dirinfo->scratch, volinfo->dataarea + ((dirinfo->currentcluster - 2) * volinfo->secperclus) + dirinfo->currentsector, 1)) return DFS_ERRMISC;
		}
	}
	...
	if (dirent->name[0] == 0)	// no more files in this directory
	{
		...
		if (dirinfo->flags & DFS_DI_BLANKENT) return DFS_OK;
		else return DFS_EOF;
	}

	//kprintf("DFS_GetNext - name[0] = $%X - long name = %s", dirent->name[0], ((dirent->attr & ATTR_LONG_NAME) == ATTR_LONG_NAME) ? "true" : "false");

	if (dirent->name[0] == 0xe5) dirent->name[0] = 0;			// handle deleted file entries
	else if ((dirent->attr & ATTR_LONG_NAME) == ATTR_LONG_NAME) dirent->name[0] = 0;
	else if (dirent->name[0] == 0x05) dirent->name[0] = 0xe5;	// handle kanji filenames beginning with 0xE5

	//kprintf("DFS_GetNext - dirent->name: %s", (char*)dirent->name);
...
*/
u32 DFS_GetFreeDirEnt(PVOLINFO volinfo, u8 *path, PDIRINFO di, PDIRENT de)
{
	u32 tempclus,i;
	
	if (DFS_OpenDir(volinfo, path, di)) return DFS_NOTFOUND;
	...
	do 
	{
		...
		else if (tempclus == DFS_ALLOCNEW) 
		{
			...
			if (tempclus == 0x0ffffff7) return DFS_ERRMISC;
			...
			{
				if (DFS_WriteSector(volinfo->unit, di->scratch, volinfo->dataarea + ((tempclus - 2) * volinfo->secperclus) + i, 1)) return DFS_ERRMISC;
			}
...
u32 DFS_OpenFile(PVOLINFO volinfo, u8 *path, u8 mode, u8 *scratch, PFILEINFO fileinfo)
{
	...
	{
		//kprintf("DFS_OpenFile - de.name: %s - fname: %s", (char*)de.name, (char*)filename);

		if (!memcmp(de.name, filename, 11)) 
		{
			// You can't use this function call to open a directory.
			if (de.attr & ATTR_DIRECTORY) return DFS_NOTFOUND;
			...
			// opened for writing.
			if (di.currentcluster == 0) fileinfo->dirsector = volinfo->rootdir + di.currentsector;
			else fileinfo->dirsector = volinfo->dataarea + ((di.currentcluster - 2) * volinfo->secperclus) + di.currentsector;

			fileinfo->diroffset = di.currententry - 1;
			
			if (volinfo->filesystem == FAT32) 
			{
				fileinfo->cluster = (u32) de.startclus_l_l |
								   ((u32) de.startclus_l_h) <<  8 |
								   ((u32) de.startclus_h_l) << 16 |
								   ((u32) de.startclus_h_h) << 24;
			}
			...
			fileinfo->filelen = (u32) de.filesize_0 |
								((u32) de.filesize_1) <<  8 |
								((u32) de.filesize_2) << 16 |
								((u32) de.filesize_3) << 24;
	...
	if (mode & DFS_WRITE) 
	{
		...
		//kprintf("DFS_OpenFile - fat1: %lu - de.name: %s - filename: %s", volinfo->fat1, (char*)de.name, (char*)filename);
		...
		if (DFS_ReadSector(volinfo->unit, scratch, fileinfo->dirsector, 1)) return DFS_ERRMISC;
...
u32 DFS_ReadFile(PFILEINFO fileinfo, u8 *scratch, u8 *buffer, u32 *successcount, u32 len)
{
	...
	if (len > fileinfo->filelen - fileinfo->pointer) len = fileinfo->filelen - fileinfo->pointer;
	...
	*successcount = 0;
	...
	while (remain && result == DFS_OK) 
	{
		...
		*successcount += bytesread;
		...
		if (div(fileinfo->pointer - bytesread, fileinfo->volinfo->secperclus * SECTOR_SIZE).quot != div(fileinfo->pointer, fileinfo->volinfo->secperclus * SECTOR_SIZE).quot) 
		{
...
void DFS_Seek(PFILEINFO fileinfo, u32 offset, u8 *scratch)
{
	...
	if (div(fileinfo->pointer, fileinfo->volinfo->secperclus * SECTOR_SIZE).quot == div(fileinfo->pointer + offset, fileinfo->volinfo->secperclus * SECTOR_SIZE).quot) 
	{
	...
	else 
	{
		...
		fileinfo->pointer = div(fileinfo->pointer, fileinfo->volinfo->secperclus * SECTOR_SIZE).quot * fileinfo->volinfo->secperclus * SECTOR_SIZE;

		// seek by clusters
		// larwe 9/30/06 bugfix changed .rem to .quot in both div calls
		while (div(fileinfo->pointer, fileinfo->volinfo->secperclus * SECTOR_SIZE).quot != div(fileinfo->pointer + offset, fileinfo->volinfo->secperclus * SECTOR_SIZE).quot)
		{
...
u32 DFS_UnlinkFile(PVOLINFO volinfo, u8 *path, u8 *scratch)
{
	...
	if (DFS_OK != DFS_OpenFile(volinfo, path, DFS_READ, scratch, &fi)) return DFS_NOTFOUND;
...
u32 DFS_WriteFile(PFILEINFO fileinfo, u8 *scratch, u8 *buffer, u32 *successcount, u32 len)
{
	...
	if (!(fileinfo->mode & DFS_WRITE)) return DFS_ERRMISC;
	...
	while (remain && result == DFS_OK) 
	{
		...
		if (div(fileinfo->pointer, SECTOR_SIZE).rem) 
		{
		   	...
		   	if (remain >= (SECTOR_SIZE - tempsize)) 
			{
				...
				if (!result) result = DFS_WriteSector(fileinfo->volinfo->unit, scratch, sector, 1);
		...
		*successcount += byteswritten;
		...
		// check to see if we stepped over a cluster boundary
		if (div(fileinfo->pointer - byteswritten, fileinfo->volinfo->secperclus * SECTOR_SIZE).quot != div(fileinfo->pointer, fileinfo->volinfo->secperclus * SECTOR_SIZE).quot) 
		{
		  	u32 lastcluster;
