...
void SystemInit(bool hardReset)
{
    VDP_setEnable(FALSE);
    SYS_disableInts();
    SetupExceptions();
    ...
    if (CFG_LoadData()) 
    {
        Stdout_Push(" \e[91mFailed to load config file!\e[0m\n");
        CFG_SaveData();
    }
    ...
    #if (HALT_Z80_ON_IO != 0)
    Stdout_Push("\n \e[91mWarning: HALT_Z80_ON_IO is enabled");
    Stdout_Push("\n in SGDK! This may cause issues!\e[0m\n\n");
    #endif 
    #if (HALT_Z80_ON_DMA != 0)
    Stdout_Push("\n \e[91mWarning: HALT_Z80_ON_DMA is enabled");
    Stdout_Push("\n in SGDK! This may cause issues!\e[0m\n\n");
    #endif
